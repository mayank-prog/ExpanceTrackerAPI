...
function showPayBtn(res) {
  if (res) {
    document.getElementById("pay-btn").innerHTML = "<h3 class='text-success'>You are a prime member</h3>";
     let html_leaderboard = `<h3>-----Our Leaderboard-----<h3>`;
     GET_leaderboard();

    document.getElementById("leaderboard-text").innerHTML = html_leaderboard;
    ...
    let html_leaderboard = `<h3>-----Our Leaderboard-----<h3>`;
    html_leaderboard += '<h6 class="text p-4">Our Leaderboard only for prime member<h6>'
    document.getElementById("leaderboard-text").innerHTML = html_leaderboard;
...
async function GET_leaderboard() {
    try {
    const token = localStorage.getItem("token");
    const res = await axios.get('http://44.204.236.201:8081/prime/', { headers: { 'Authorization': token } });
      if(res){
        console.log(res)
        Show_leaderboard(res.data);
      }
  } catch (err) {
    console.log(err)
  }
